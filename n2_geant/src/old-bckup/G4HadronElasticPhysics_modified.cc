...
// ********************************************************************
//
// $Id: G4HadronElasticPhysics_modified.cc,v 1.1 2011-06-14 15:43:41 monasoft Exp $
// GEANT4 tag $Name:  $
//
//---------------------------------------------------------------------------
//
...
#include "G4HadronElasticPhysics_modified.hh"
...
//removed zwk because obselete in geant4 4.9.3
...
{
  G4cout << "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" << G4endl;
  G4cout << "Using modified elastic physics list" << G4endl;
  G4cout << "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" << G4endl;
  G4cout << "Removing neutron elastic collisions to avoid conflicts with MENATE_R" << G4endl;
  G4cout << "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" << G4endl;

  if(wasActivated) return;
  wasActivated = true;

  //Is obsolete in current G4 code.
  ...
  while( (*theParticleIterator)() )
  {
	...
	hel = new G4HadronElasticProcess("hElastic");
      ...
      //start comment here to remove normal neutron interaction
      /*
      // neutron case
	...
	neutronModel = new G4LElastic();
      ...
      if(hpFlag) {
	neutronModel->SetMinEnergy(19.9*MeV);
	neutronHPModel = new G4NeutronHPElastic();
	neutronHPModel->SetMinEnergy(0*MeV);
	neutronHPModel->SetMaxEnergy(20*MeV);
	...
	G4cout << "### HadronElasticPhysics added for " 
	       ...
	       //End comment here to remove normal neutron interaction.
	       */
