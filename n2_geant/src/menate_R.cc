 ...
 if(!getenv("MENATEG4XS")) 
   {
     ...
     G4Exception("Program aborted in menate::ReadCrossSectionFile()","",severity,"");
   }
 G4String  DirName = getenv("MENATEG4XS");
  // G4String DirName = "/projects/mona-sim/MENATE_R_CrossSections";
  //G4String DirName = "/projects/mona-sim/jonesm/menate_r";
  //
 ...
 FileName = DirName+"/"+FileName;
  ...
  if(theFile.good())
      ...
      for(G4int k=0; k<NumberOfLines; k++)
	{
      ...
      double scale[6] = {0.52,1,1,1,1,1};//0.520
      //double scale[6] = {1,1,10,10,1,1};, S = 0.4 will do an ok job for vrel > 10
      //double scale[6] = {0.1,1,3,1,1,1};
      //double scale[6] = {0.857207,0.0711429,1,1,4.26405,1};
      //double scale[6] = {1,0.0518795,1,0.103057,0.231589,0.0264011};
      //double scale[6] = {0.52778,0.0704,3,0.7,1,0.2778}; // increase nng4 to 3 for good fit
      // use this one
      //double scale[6] = {0.133,1,4,2,1,1};

      // slots of comparable magnitude: 2,3,4
       

      for(int i=0;i<6;i++){
            scale[i] = scale[i]*1.0;//0.4
      }
      /*scale[0] = scale[0]*1.7;
      scale[1] = scale[1]*1.7;
      scale[3] = scale[3]*1.7;
      ...
      // ranked in order of largest to smallest
	  if(FileName.contains("nng4")){
	    theReactionXS[k].SetTotalCrossSection(scale[2]*theCrossSection*barn);//5
	    G4cout << "nng4 cross section enhanced :) !!" << G4endl;
	  }
      else if(FileName.contains("na9Be")){
        theReactionXS[k].SetTotalCrossSection(scale[5]*theCrossSection*barn);
	    G4cout << "na9Be cross section enhanced :) !!" << G4endl;
      }
      else if(FileName.contains("np12B")){
        theReactionXS[k].SetTotalCrossSection(scale[1]*theCrossSection*barn);
	    G4cout << "np12B cross section enhanced :) !!" << G4endl;
      }
      else if(FileName.contains("nnp11B")){
        theReactionXS[k].SetTotalCrossSection(scale[0]*theCrossSection*barn);
	    G4cout << "nnp11B cross section enhanced :) !!" << G4endl;
      }
      else if(FileName.contains("2n11C")){
        theReactionXS[k].SetTotalCrossSection(scale[4]*theCrossSection*barn);
	    G4cout << "2n11C cross section enhanced :) !!" << G4endl;
      }
      else if(FileName.contains("nn3a")){
        theReactionXS[k].SetTotalCrossSection(scale[3]*theCrossSection*barn);//5
	    G4cout << "nn3a cross section enhanced :) !!" << G4endl;
      }
      /*else if(FileName.contains("Hydrogen1_el")){
        theReactionXS[k].SetTotalCrossSection(2.0*theCrossSection*barn);//5
	    G4cout << "nn3a cross section enhanced :) !!" << G4endl;
      }*/
...
G4double menate_R::GetCrossSection(G4double KinEng, CrossSectionClass* theReactionXS)
{
  ...
  G4int NumberOfLines;
    ...
    G4cout << "Neutron KE: " << KinEng << G4endl;
  ...
  while(Remaining_Eng > 4.439*MeV)
    {
      ...
      //Remaining_Eng -= 0.400*MeV; // MDJ 3/10/15
    }
...
G4VParticleChange* menate_R::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep)
{
  ...
  if(isnan(thePosition.x())||isnan(thePosition.y())||isnan(thePosition.z())){ 
 ...
 else if(ReactionName == "N_C12_NGamma")
   {
     ...
     if(KinEng_Int >= 8.5*MeV)
       //{ Available_Eng = 8.5*MeV*(G4UniformRand()); } // Max available Eng = 8.5 MeV ?
        { Available_Eng = 10*MeV*(G4UniformRand()); } // MDJ 3/10/15
     else
	...
	/* if(Num_gamma_4439k > 1 )
	   {	  
         // G4cout << "Double Gamma! " << G4endl;
	     G4double T_Gamma = 4.439*MeV;
	     G4double theta_Gamma = acos(1.-(2.*G4UniformRand()));
	     G4double phi_Gamma = 2.*Pi*G4UniformRand();
         ...
         G4double T_Gamma2 = 4.439*MeV;
	     G4double theta_Gamma2 = acos(1.-(2.*G4UniformRand()));
	     G4double phi_Gamma2 = 2.*Pi*G4UniformRand();
	     
	     G4ThreeVector MomDir_Gamma2;
	     MomDir_Gamma2 = GenMomDir(MomDir_Int,theta_Gamma2,phi_Gamma2);

	     // Generate a Secondary Gamma (4.439 MeV)
	     G4DynamicParticle* theSecGamma2 = new G4DynamicParticle;
	     G4ParticleDefinition* theGammaDefinition2 = G4Gamma::Gamma();
	     theSecGamma2->SetDefinition(theGammaDefinition2);
	     theSecGamma2->SetKineticEnergy(T_Gamma2);
	     theSecGamma2->SetMomentumDirection(MomDir_Gamma2);
         ...
         //G4cout << "track23" << G4endl;

	     G4Track* theGammaTrack2 = new G4Track(theSecGamma2,GlobalTime,thePosition);
         //G4cout << "theTouchable" << G4endl;
	     theGammaTrack2->SetTouchableHandle(theTouchable);
         //G4cout << "GammaTrack2" << G4endl;
	     aParticleChange.AddSecondary(theGammaTrack2);
