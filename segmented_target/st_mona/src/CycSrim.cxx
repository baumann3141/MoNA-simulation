...
Int_t CycSrim::Init() {
  ...
  //add zwk 9/26/12
  //used for range correction based on mass of original SRIM clac.
  TGeoManager *fGeom = new TGeoManager("geom","radionuclides");
  TGeoElementTable *fTable = fGeom->GetElementTable();
...
//   srimFile = Form("/projects/mona-sim/jonesm/st_mona/bin/srim/%s",srimfile[fMaterial]); cout << "DataDir: " << srimFile << endl;
   ...
   for(z=1;z<93;z++) {
      ...
      fNumStopping[z-1] = num;

      //use TGeoElement to get mass used in the original SRIM calcuation
      //for the range, then scale the range relative to that
      TGeoElement *u = fTable->GetElement(z);
      Double_t a_used_calc = u->A();
	 ...
	 stop_p->a_used_calc            = a_used_calc;
	 //printf("z:%d  a_used:%f  %f\n",z,stop_p->a_used_calc, a_used_calc);
         }
      }
...
Double_t CycSrim::GetEnergyLossFromResidual(Int_t z, Double_t a, Double_t e) {
   ...
   for(Int_t ie=0;ie<numE;ie++) {
      stoppingStruct *stop_p = (stoppingStruct*)fStopping[z-1][ie];     
      a_used_calc = stop_p->a_used_calc;
      Double_t range_corr = a/a_used_calc;
      evals[ie] = stop_p->eOverA;
      ranges[ie] = range_corr * stop_p->range;
      }
...
Double_t CycSrim::GetResidualEnergy(Int_t z, Double_t a, Double_t e) {
   ...
   // 10 is the MAXIMUM number for which a bus error is avoided
...
//    printf("Range = %f, thickness = %f, gtr = %f\n",range,fThickness,gtr);
   ...
   if(gtr < 0.0) {
      ...
      eOverAOut = Interp(ranges,evals,numE,4,gtr);
      }
   else {
      eOverAOut = Interp(ranges,evals,numE,4,gtr);
      }
...
Double_t CycSrim::GetRange(Int_t z, Double_t a, Double_t e, Bool_t lengthUnits) {
   ...
   Double_t range = Interp(evals,ranges,numE,10,e_a);//2
...
const Char_t*  CycSrim::GetSrimDataDir() const {
  ...
  //TString CYC_SRIM_DATA_DIR="../bin";
  //TString CYC_SRIM_DATA_DIR = "/projects/mona-sim/jonesm/st_mona/bin";
  TString CYC_SRIM_DATA_DIR = "/projects/mona-sim/baumann/st_mona/bin";
