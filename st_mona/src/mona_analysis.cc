...
int MonaAnalyzedEvent::ana(CalMonaEvent* data) {
	    ...
	    // Ragney Implementation
 	    ...
 	    energy = fRageny.energy(matTarget,range + dTarg/2,&p);
            ...
            */
            ...
            deltaE[1] = p.getA()*(fRageny.energy(srimTarget,dTarg/2,&p));
            //cout << p.getA() << endl;
            //cout << fRageny.energy(srimTarget, dTarg/2, &p) << endl;
            //cout << "deltaE[1]: " << deltaE[1] << " srimTarget: " << srimTarget << " dTarget: " << dTarg <<  "e0: " << e0 << endl;
            #endif 
  ...
  // CycSrim Implementation 
	...
	//EDecayG2 = temp_G;
      ...
      // Dalitz plot masses
	/*  lvT = lvG1N + lvF;
      Dalitz_mcn1 = (pow(lvT.M(),2) - pow(lvF.M() + lvG1N.M(),2) ) / ( pow(lvF.M()+lvG1N.M()+EDecayTotalG1and2,2) - pow(lvF.M() + lvG1N.M(),2) );


	  lvT = lvG2N + lvF;
      edecayLow = lvT.M() - lvG2N.M() - lvF.M();
      Dalitz_mcn2 = (pow(lvT.M(),2) - pow(lvF.M() + lvG2N.M(),2) ) / ( pow(lvF.M()+lvG2N.M()+EDecayTotalG1and2,2) - pow(lvF.M() + lvG2N.M(),2) );

	
      lvT = lvG1N + lvG2N;
	  Dalitz_mnn = (pow(lvT.M(),2) - pow(lvG1N.M() + lvG2N.M(),2) ) / ( pow(lvG1N.M()+lvG2N.M()+EDecayTotalG1and2,2) - pow(lvG1N.M() + lvG2N.M(),2) );
    */
	...
	*/
    ...
    JacobiCorr(lvF, lvN, lvN2, massF, massN1, massN2, TsystemR, YsystemR, cosn1n2theta, n1pp, n2pp, n1pt, n2pt);
	...
	Tsys_Ex_Et1_noRes = TsystemR[0];
	Tsys_Ex_Et2_noRes = TsystemR[2];
	Ysys_Ex_Et1_noRes = YsystemR[0];
	Ysys_Ex_Et2_noRes = YsystemR[2];

	Tsys_CosTheta1_noRes = TsystemR[1];
	Tsys_CosTheta2_noRes = TsystemR[3];
	Ysys_CosTheta1_noRes = YsystemR[1];
	Ysys_CosTheta2_noRes = YsystemR[3];
...
int main (size_t argc, char* argv[] ) {
 ...
 // Read in input file here
  ...
  string INPUT_DATA_DIR = "/projects/mona-sim/baunamm/st_mona/input_files/";
  ...
  //INPUT_FILE    = "N22_input_test.txt";
  //INPUT_FILE    = "O25_input.txt";
	  ...
	  // Remove whitespace from var_name and var_value
	     ...
	     // List doubles to set and check name of variable        
                  if( strcmp(var_name,"dTarget") == 0 )   	target_thickness = dvalue;
								...
								//tflatfoil *= 100*1420;
								cout << " TFLATFOIL: " << tflatfoil << endl;
  								//cout << " dvalue: " << dvalue << endl;
  								//cout << "math: " << dvalue*10000*171.5 << endl;
							}
	     else if( strcmp(var_name, "maxbulge") == 0) {	 bulge 		= dvalue;
								cout << "BULGE: " << bulge << endl;
							}
...
// Set arguments afterwards so that we can overwrite input file (if we so desire)
 ...
 e->bulge = bulge;
  // implement selecting target from input file?
...
#include "maps.icc"
    ...
    else if ( strcmp(COSYmap, "m24O_Jones320A") == 0)   e->setMaps(m24O_Jones320A, m24O_Jones320Ai);
    else if ( strcmp(COSYmap, "m22O_Jones320A") == 0) e->setMaps(m22O_Jones320A, m22O_Jones320Ai);
    else if ( strcmp(COSYmap, "m23O_Jones320A") == 0) e->setMaps(m23O_Jones320A, m23O_Jones320Ai);
    else if ( strcmp(COSYmap, "m21N_Jones320A") == 0) e->setMaps(m21N_Jones320A, m21N_Jones320Ai);
    else if ( strcmp(COSYmap, "m22N_Jones320A") == 0) e->setMaps(m22N_Jones320A, m22N_Jones320Ai);
    	else if( strcmp(COSYmap, "m24O_redpath") == 0) e->setMaps(m24O_redpath, m24O_redpathi);
    	else if( strcmp(COSYmap, "m22O_redpath") == 0) e->setMaps(m22O_redpath, m22O_redpathi);
 ...
 //  You are welcome to change this as you see fit.
    ...
    cout << "tflatfoil: " << e->tflatfoil << endl;
	// You may have to add the foil!!!
       if (frag == "23O")   		e->setCycSrim(srimBe);
					...
					e->setCycSrim(srimBe);// temporarily Be target
					e->setSrimFoil(Kapton);
				}
  else if (frag == "21N_Jones"){
                    e->setCycSrim(srimBe);
                    e->setSrimFoil(Kapton);
       }
	...
	e->setCycSrim(srimBe);
  	...
  	INFO("Using settings for fragment O24 from e15118");
	e->setCycSrim(srimBe);
  }
  else if (frag == "O22") {
  	INFO("Using settings for fragment O22 from e15118");
	e->setCycSrim(srimBe);
  }
  else {
      ERR("Unknown fragment %s",frag.c_str());
        e->setCycSrim(srimBe);
  }
  ...
  while( data.getEvent() ) { // as long as there are good events
    ...
    for (size_t i = 0; i <= nMix; i++) {
      ...
      //cout << "analysing frag from old mona" << endl;
